...
app.post("/createshoe", (req, res) => {
  ...
  db.query(
    "INSERT INTO giay (id_kieugiay, tengiay, tenmagiay, hinhanh, giaban, tinhtrang, noidung) VALUES (?,?,?,?,?,?,?)",
    [id_kieugiay, tengiay, tenmagiay, hinhanh, giaban, tinhtrang, noidung],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.post("/createshoedetail", (req, res) => {
  ...
  db.query(
    "select * from chitietgiay where id_giay = ? and id_size = ? and id_color = ?",
    [id_giay, id_size, id_color],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.get("/user", (req, res) => {
  db.query("SELECT hoten,diachi,sodienthoai,email,username,role FROM user;", (err, result) => {
    if (err) {
      console.log(err);
    } else {
      res.send(result);
...
app.put("/updateshoe", (req, res) => {
  ...
  const noidung = req.body.noidung;
  db.query(
    "UPDATE giay SET tengiay = ?, tenmagiay = ?, hinhanh = ? , giaban = ?, tinhtrang = ?, noidung = ?, id_kieugiay = ? WHERE id_giay = ?",
    [tengiay, tenmagiay, hinhanh, giaban, tinhtrang, noidung, id_kieugiay, id_giay],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.get("/relateproduct/:id", (req, res) => {
  const id = req.params.id
  db.query("select * from giay a left join khuyenmai b on a.id_khuyenmai = b.id_khuyenmai  where id_kieugiay = ? and tinhtrang = '1' ORDER BY RAND() limit 5", id, (err, result) => {
    if (err) {
      console.log(err)
    } else {
      res.send(result);
...
app.get("/shoeontype/:id", (req, res) => {
  const id = req.params.id
  db.query("select * from giay a left join khuyenmai b on a.id_khuyenmai = b.id_khuyenmai where tinhtrang = '1' and id_kieugiay = ?", id, (err, result) => {
    if (err) {
      console.log(err)
    } else {
      res.send(result)
...
app.get("/activeshoe", (req, res) => {
  db.query("SELECT * FROM giay a left join khuyenmai b on a.id_khuyenmai = b.id_khuyenmai where tinhtrang='1'", (err, result) => {
    if (err) {
      console.log(err);
    } else {
      res.send(result);
...
app.put("/updatesalestatus", (req, res) => {
  ...
  const id_khuyenmai = req.body.id_khuyenmai;
  db.query(
    "UPDATE khuyenmai SET trangthai = ?, phantramkhuyenmai = 0 WHERE id_khuyenmai = ?",
...
app.post("/uploadbannerurl", (req, res) => {
  ...
  const title_banner = req.body.title_banner;
  const sub_title_banner = req.body.sub_title_banner;
  const title_effect = req.body.title_effect;
  const sub_effect = req.body.sub_effect;
  const button_effect = req.body.button_effect;
  db.query("INSERT INTO banner (url_banner,id_tintuc,title_banner,sub_title_banner,title_effect,sub_effect,button_effect) values(?,?,?,?,?,?,?)",
    [imageURL, id_tintuc, title_banner, sub_title_banner, title_effect, sub_effect, button_effect], (err, result) => {
      if (err) {
        console.log(err)
        res.send('error');
      } else {
...
app.get("/banner", (req, res) => {
  db.query("select * from banner a left join tintuc b on a.id_tintuc = b.id_tintuc", (err, result) => {
    if (err) {
      console.log(err)
    } else {
      res.send(result)
...
app.post("/sendmailtouser", (req, res) => {
  ...
  cartItems.forEach(item => {
    htmlContent = htmlContent + `
      <div><img height='100px' width='150px' src="http://hieugiaynhuy.tk/gallery/${item.hinhanh}" />
  ...
  db.query("select * from user where role = 'admin'", (err, result) => {
    if (err) {
      ...
      const mailForAdmin = {
        ...
        html: `<h3>Đơn hàng được tạo vào: ${Date()}</h3><p>Tên khách hàng : ${hoten}</p><p>Số điện thoại: ${sodienthoai}</p><p style='color:red'>Chi tiết xem tại http://hieugiaynhuy.tk/readorder/></p>`
      };
...
//GET NEWS HOME
app.get("/newshome", (req, res) => {
  db.query("select * from tintuc order by id_tintuc desc limit 3", (err, result) => {
    if (err) {
      console.log(err)
    } else {
      res.send(result)
...
app.put("/updatebanner", (req, res) => {
  ...
  const sub_effect = req.body.sub_effect;
  const button_effect = req.body.button_effect;
  db.query("UPDATE banner SET url_banner = ?, id_tintuc = ?, title_banner = ?, sub_title_banner = ?, title_effect = ?, sub_effect = ?, button_effect = ? where id_banner = ?",
    [imageURL, id_tintuc, title_banner, sub_title_banner, title_effect, sub_effect, button_effect, id_banner], (err, result) => {
      if (err) {
        console.log(err)
        res.send('error');
      } else {
...
app.get("/city", (req, res) => {
  db.query(
    "SELECT * FROM province order by _name asc",(err, result) => {
      if (err) {
        console.log(err);
      } else {
        res.send(result)
...
app.get("/city/:id", (req, res) => {
  const id = req.params.id
  db.query(
    "select * from district where _province_id = ? order by _name asc",id,(err, result) => {
      if (err) {
        console.log(err);
      } else {
        res.send(result)
...
//GET WARD
app.get("/city/district/:id", (req, res) => {
  const id = req.params.id
  db.query(
    "select * from ward where _district_id = ? order by _name asc",id,(err, result) => {
      if (err) {
        console.log(err);
      } else {
        res.send(result)
...
app.put("/updatestar", (req, res) => {
  ...
  const numOfCmt = req.body.numOfCmt;
  const id_giay = req.body.id_giay;
  db.query(
    "UPDATE giay SET sosao = ?, sobinhluan = ? WHERE id_giay = ?",
    [star, numOfCmt,id_giay],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
